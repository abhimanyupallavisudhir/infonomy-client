...
READINFOOFFERSFORDECISIONCONTEXTQUESTIONSCONTEXTIDANSWERSGET200RESPONSEINNER_ANY_OF_SCHEMAS = ["InfoOfferReadPrivate", "InfoOfferReadPublic"]

class ReadInfoOffersForDecisionContextQuestionsContextIdAnswersGet200ResponseInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ReadInfoOffersForDecisionContextQuestionsContextIdAnswersGet200ResponseInner.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ReadInfoOffersForDecisionContextQuestionsContextIdAnswersGet200ResponseInner with anyOf schemas: InfoOfferReadPrivate, InfoOfferReadPublic. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ReadInfoOffersForDecisionContextQuestionsContextIdAnswersGet200ResponseInner with anyOf schemas: InfoOfferReadPrivate, InfoOfferReadPublic. Details: " + ", ".join(error_messages))
        else:
            return instance
